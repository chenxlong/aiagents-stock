...
class SectorStrategyDataFetcher:
    ...
    def __init__(self):
        ...
        self.logger.debug("[智策] 板块数据获取器初始化...")
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    self.logger.warning(f" 请求失败，{self.retry_delay}秒后重试... (尝试 {attempt + 1}/{self.max_retries})")
    ...
    def _get_sector_performance(self):
        ...
        try:
            ...
            self.logger.info(f" [Akshare] 成功获取 {len(df)} 条行业板块行情数据:\n {df}")
    ...
    def _get_concept_performance(self):
        ...
        try:
            ...
            self.logger.info(f" [Akshare] 成功获取 {len(df)} 条概念板块行情数据:\n {df}")
    ...
    def _get_sector_fund_flow(self):
        ...
        try:
            ...
            self.logger.info(f" [Akshare] 成功获取 {len(df)} 条行业资金流向数据:\n {df}")
