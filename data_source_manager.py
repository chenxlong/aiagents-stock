...
class DataSourceManager:
    ...
    def get_stock_hist_data(self, symbol, start_date=None, end_date=None, adjust='qfq'):
        ...
        try:
            ...
            self.logger.info(f"[Akshare] 正在获取 {symbol} 的历史数据...")
            ...
            if df is not None and not df.empty:
                ...
                self.logger.info(f"[Akshare] ✅ 成功获取 {len(df)} 条数据")
                return df
        except Exception as e:
    ...
    def get_realtime_quotes(self, symbol):
        ...
        try:
            import akshare as ak
            self.logger.info(f"[Akshare] 正在获取 {symbol} 的实时行情...")
            ...
            if not stock_df.empty:
                ...
                self.logger.info(f"[Akshare] ✅ 成功获取实时行情")
                return quotes
        except Exception as e:
            self.logger.error(f"[Akshare] ❌ 获取失败: {e}")
    ...
    def get_financial_data(self, symbol, report_type='income'):
        ...
        try:
            import akshare as ak
            self.logger.info(f"[Akshare] 正在获取 {symbol} 的财务数据...")
            ...
            if df is not None and not df.empty:
                self.logger.info(f"[Akshare] ✅ 成功获取财务数据")
                return df
        except Exception as e:
            self.logger.error(f"[Akshare] ❌ 获取失败: {e}")
