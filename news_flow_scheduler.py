...
class NewsFlowScheduler:
    ...
    def __init__(self):
        ...
        self.logger.info("[新闻流量] 调度器初始化完成")
    ...
    def _get_engine(self):
        ...
        if self.engine is None:
            try:
                ...
                self.logger.error(f"引擎模块初始化失败: {e}")
        return self.engine
    
    def start(self):
        """启动调度器"""
        if self.running:
            self.logger.warning("[新闻流量] 调度器已在运行")
            return
        ...
        with self.lock:
            ...
            if self.task_enabled.get('sync_hotspots'):
                ...
                self.logger.info(f"[新闻流量] 注册热点同步任务，间隔{interval}分钟")
            ...
            if self.task_enabled.get('generate_alerts'):
                ...
                self.logger.info(f"[新闻流量] 注册预警生成任务，间隔{interval}分钟")
            ...
            if self.task_enabled.get('deep_analysis'):
                ...
                self.logger.info(f"[新闻流量] 注册深度分析任务，间隔{interval}分钟")
    ...
    def _schedule_loop(self):
        ...
        while self.running:
            try:
                ...
                self.logger.error(f"[新闻流量] 调度循环异常: {e}")
    ...
    def _run_sync_hotspots(self):
        ...
        self.logger.info(f"[新闻流量] 开始执行: {task_name}")
        ...
        try:
            ...
            if result.get('success'):
                ...
                self.logger.info(f"[新闻流量] {task_name}完成: {message}")
            else:
                message = result.get('error', '未知错误')
                self._log_task(task_name, task_type, 'failed', message, duration)
                self.logger.error(f"[新闻流量] {task_name}失败: {message}")
            ...
            self.logger.error(f"[新闻流量] {task_name}异常: {e}")
    ...
    def set_task_enabled(self, task_type: str, enabled: bool):
        ...
        if task_type in self.task_enabled:
            ...
            self.logger.info(f"[新闻流量] 任务 {task_type} {'启用' if enabled else '禁用'}")
    ...
    def set_task_interval(self, task_type: str, interval: int):
        ...
        if task_type in self.task_intervals:
            ...
            self.logger.info(f"[新闻流量] 任务 {task_type} 间隔设置为 {interval} 分钟")
